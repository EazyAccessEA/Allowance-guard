...
const nextConfig = {
  ...
  async headers() {
    return [
      ...
      // Immutable caching only for fingerprinted/versioned assets.
      // Applying it globally makes cache-busted re-requests from a
      // single client look bot-like to edge DDoS layers.
      {
        source: '/_next/static/(.*)',
        headers: [
          {
