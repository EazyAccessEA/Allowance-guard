...
export default function ApiReferencePage() {
  return (
    <div className="min-h-screen bg-[#0A0E1A] py-16 sm:py-24">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-12">
          <h1 className="text-4xl sm:text-5xl font-bold text-white tracking-tight" style={headingFont}>API Reference</h1>
          ...
          <div className="flex flex-wrap gap-3 mt-6">
            ...
            <span className="px-3 py-1 bg-slate-800/40 text-slate-400 text-sm rounded-full border border-slate-700/50">
              JSON responses
            </span>
            <span className="px-3 py-1 bg-slate-800/40 text-slate-400 text-sm rounded-full border border-slate-700/50">
              Bearer token auth
        ...
        {/* Authentication */}
        <div className="mb-12">
          <h2 className="text-2xl font-bold text-slate-100 mb-4 flex items-center gap-2" style={headingFont}>
            <Key className="w-6 h-6 text-amber-400" /> Authentication
          </h2>
          <p className="text-slate-400 mb-4">
            All API endpoints (except <code className="text-sm bg-[#0A0E1A] border border-slate-700/50 px-1.5 py-0.5 rounded text-slate-300">/health</code>)
            require an API key sent via the <code className="text-sm bg-[#0A0E1A] border border-slate-700/50 px-1.5 py-0.5 rounded text-slate-300">Authorization</code> header.
          </p>
          <CodeExample
            tabs={[
              {
...
data = res.json()['data']`,
        ...
        <div className="mb-12">
          <h2 className="text-2xl font-bold text-slate-100 mb-4 flex items-center gap-2" style={headingFont}>
            <Zap className="w-6 h-6 text-amber-400" /> Rate Limits
          </h2>
          <p className="text-slate-400 mb-4">
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-slate-800/50 text-left">
                  ...
                  <th className="px-4 py-3 font-medium text-slate-400">Daily Limit</th>
                  <th className="px-4 py-3 font-medium text-slate-400">Burst (per min)</th>
                  <th className="px-4 py-3 font-medium text-slate-400">Price</th>
                </tr>
              </thead>
              <tbody>
                {rateLimits.map((r) => (
                  <tr key={r.plan} className="border-t border-slate-700/30">
                    <td className="px-4 py-3 text-slate-100 font-medium">{r.plan}</td>
                    <td className="px-4 py-3 font-mono text-slate-400">{r.daily}</td>
                    <td className="px-4 py-3 font-mono text-slate-400">{r.burst}</td>
                    <td className="px-4 py-3 text-slate-400">{r.price}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <p className="text-xs text-slate-500 mt-3">
            Headers: <code className="bg-[#0A0E1A] border border-slate-700/50 px-1 rounded text-slate-400">X-RateLimit-Limit</code>,{' '}
            <code className="bg-[#0A0E1A] border border-slate-700/50 px-1 rounded text-slate-400">X-RateLimit-Remaining</code>
          </p>
        </div>
        ...
        <div className="mb-12">
          <h2 className="text-2xl font-bold text-slate-100 mb-4 flex items-center gap-2" style={headingFont}>
            <Activity className="w-6 h-6 text-amber-400" /> Response Format
          </h2>
          <p className="text-slate-400 mb-4">
            All endpoints return a consistent JSON envelope with{' '}
            <code className="text-sm bg-[#0A0E1A] border border-slate-700/50 px-1.5 py-0.5 rounded text-slate-300">data</code>,{' '}
            <code className="text-sm bg-[#0A0E1A] border border-slate-700/50 px-1.5 py-0.5 rounded text-slate-300">error</code>, and{' '}
            <code className="text-sm bg-[#0A0E1A] border border-slate-700/50 px-1.5 py-0.5 rounded text-slate-300">meta</code> fields.
          </p>
          <CodeExample
            tabs={[
              {
  ...
  "meta": {
    ...
    "timestamp": "2026-03-30T12:00:00.000Z"
        ...
        <div className="mb-12">
          <h2
            className="text-2xl font-bold text-slate-100 mb-6 flex items-center gap-2"
            style={headingFont}
          >
          ...
          <h3 className="text-lg font-bold text-slate-100 mb-3" style={headingFont}>
            Health &amp; Info
          </h3>
  ...
  "meta": { ... }
        ...
        <div className="mb-12">
          <h3
            className="text-lg font-bold text-slate-100 mb-3 flex items-center gap-2"
            style={headingFont}
          >
...
curl "https://www.allowanceguard.com/api/v1/risk-score?wallet=0x1234...abcd" \\
  -H "Authorization: Bearer ag_live_your_key_here"`,
        ...
        <div className="mb-12">
          ...
          <div className="border border-slate-700/50 rounded-xl overflow-hidden">
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-slate-800/50 text-left">
                  <th className="px-4 py-3 font-medium text-slate-400">HTTP</th>
              ...
              <tbody>
                {[
                  ...
                  <tr key={code} className="border-t border-slate-700/30">
                    <td className="px-4 py-2 font-mono text-xs text-slate-400">{http}</td>
                    <td className="px-4 py-2 font-mono text-xs text-slate-100">{code}</td>
        ...
        {/* CTA */}
        <div className="text-center p-8 rounded-xl bg-slate-800/40 border border-slate-700/50">
          <h3
          ...
          <div className="flex justify-center gap-4">
            <a
              href="/account/keys"
              className="px-6 py-2.5 bg-amber-500 hover:bg-amber-400 text-[#0A0E1A] rounded-lg font-semibold transition-colors"
            >
              Get API Key
            </a>
            <a
              href="/pricing"
              className="px-6 py-2.5 bg-slate-800/60 border border-slate-700/50 text-slate-100 rounded-lg font-medium hover:bg-slate-700/60 transition-colors"
            >
              View Plans
            </a>
          </div>
