...
export async function GET(_req: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const data = transformRowsForShare(rows as unknown as { chain_id: number; token_address: string; spender_address: string; standard: string; allowance_type: string; amount: string; is_unlimited: boolean; risk_flags: string[] }[], {
