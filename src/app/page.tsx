...
/**
 * Landing page — conversion flow for the Scared Retail persona.
 *
 * Promoted from /home on 2026-04-19. The prior Coming Soon / waitlist
 * variant at this route is retired — the product is live (extensions
 * approved on Chrome + awaiting appeal on Firefox; OTP checkout
 * shipping; 27-chain scanner operational), so the landing page is
 * the scanner itself. The waitlist SubscribeForm remains available
 * at /coming-soon for future pre-announce flows.
 *
 * Order:
 *  1. Hero (plain-English threat + AddressInput above the fold)
 *  2. ChainLogoCarousel (immediate trust evidence)
 *  3. StatisticsSection (loss aversion early)
 *  4. SampleScanDemo (recognition: here's what a scan looks like)
 *  5. HowItWorks (clarity: three steps, plain copy)
 *  6. FeaturesPreview (differentiation)
 *  7. CTABand (single dark inverse moment)
 *  8. AppArea (inline when a wallet is pasted/connected)
 *
 * Cuts vs v1: Testimonials section removed entirely (no fakes).
 * Will be reinstated when real testimonials exist.
 *
 * Voice: post-Mozilla remediation rules apply — advisory only, no
 * "protect / secure / prevent / guarantee / block" framing. Hero copy
 * audited 2026-04-19 against memory/compliance-risk/claims-register.md
 * and passed.
 */
