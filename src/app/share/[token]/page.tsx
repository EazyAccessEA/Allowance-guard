...
export default function SharePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-10 bg-background-primary dark:bg-secondary-900 min-h-screen">
      {/* noindex */}
      <Head>
        <meta name="robots" content="noindex,nofollow" />
      </Head>

      <h1 className="text-2xl font-semibold text-text-primary dark:text-secondary-100">Allowance Guard — Shared Report</h1>
      {error && <div className="mt-4 rounded bg-red-50 dark:bg-red-900/20 px-3 py-2 text-sm text-red-700 dark:text-red-300">{error}</div>}
      {!data && !error && <div className="mt-4 text-sm text-text-tertiary dark:text-secondary-400">Loading…</div>}
      ...
      {data && (
        <>
          ...
          <p className="mt-4 text-xs text-text-tertiary dark:text-secondary-500">Read-only. Generated by Allowance Guard.</p>
        </>
      )}
    </main>
  )
