...
export default function HistoricalTimeline({ wallet }: HistoricalTimelineProps) {
 ...
 switch (type) {
 case 'approval_granted': return <Plus className="w-4 h-4 text-ink-blue" />
 case 'approval_changed': return <ArrowUpRight className="w-4 h-4 text-amber-deep" />
 case 'approval_revoked': return <Minus className="w-4 h-4 text-semantic-success-700" />
 case 'risk_changed': return <Shield className="w-4 h-4 text-crimson-paper" />
 }
 }
 ...
 {riskTrend > 0 ? (
 <TrendingUp className="w-4 h-4 text-crimson-paper" />
 ) : riskTrend < 0 ? (
 <TrendingDown className="w-4 h-4 text-semantic-success-700" />
 ) : null}
 <span className={`text-lg font-bold ${
 riskTrend > 0 ? 'text-crimson-paper' : riskTrend < 0 ? 'text-semantic-success-700' : 'text-ink'
 }`}>
 ...
 const color = s.risk_score >= 70 ? 'bg-paper-sub' :
 s.risk_score >= 40 ? 'bg-amber-500' : 'bg-paper-sub'
 return (
 <div
 key={i}
 ...
 {/* Timeline dot */}
 <div className="absolute left-2.5 mt-1.5 w-3 h-3 rounded-full border-2 border-background-primary bg-amber-deep" />
 ...
 <TrendingUp className="w-3 h-3 text-crimson-paper" />
 )}
 {evt.risk_score < evt.previous_risk_score && (
 <TrendingDown className="w-3 h-3 text-semantic-success-700" />
 )}
 </div>
 )}
