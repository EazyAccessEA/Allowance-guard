...
export default function SupportNudge({ when }: { when: 'after-revoke' | 'manual' }) {
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className="fixed bottom-4 right-4 max-w-sm rounded-xl border border-border-primary dark:border-secondary-700 bg-background-primary dark:bg-secondary-800 p-3 shadow-lg dark:shadow-dark-subtle z-50"
    >
      ...
      <button
        className="mt-2 rounded border border-border-primary dark:border-secondary-600 min-h-[44px] min-w-[44px] px-3 py-2 text-xs text-text-secondary dark:text-secondary-400 hover:text-text-primary dark:hover:text-secondary-100 hover:bg-background-secondary dark:hover:bg-secondary-700 transition-colors duration-200"
