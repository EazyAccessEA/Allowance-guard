...
export default function ActivityTimeline({ wallet }: { wallet: string }) {
 ...
 {error && (
 <div className="mt-3 rounded-lg border border-crimson-paper/40 bg-paper-sub px-4 py-3 text-sm text-crimson-paper">
 {error}
 ...
 const badge =
 r.status === 'verified' ? 'bg-semantic-success-50 text-semantic-success-700 border border-semantic-success-200 ' :
 r.status === 'pending' ? 'bg-paper-sub text-amber-deep border border-amber-deep/40 ' :
 r.status === 'mismatch' ? 'bg-semantic-warning-50 text-semantic-warning-700 border border-semantic-warning-200 ' :
 'bg-semantic-error-50 text-semantic-error-700 border border-semantic-error-200 '
 return (
 ...
 <a href={url} target="_blank" rel="noopener noreferrer" className="underline text-xs text-amber-deep hover:text-amber-deep">View on explorer</a>
 {r.status !== 'verified' && (
 <button onClick={() => verify(r.id)} className="rounded border border-ink-rule bg-paper-deep text-ink px-3 py-1.5 min-h-[32px] text-xs hover:bg-paper-sub transition-colors">Verify now</button>
 )}
 {r.error && <span className="text-xs text-semantic-error-600">Error: {r.error}</span>}
 </div>
