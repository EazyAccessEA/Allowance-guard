...
export default function TokenResults({ state }: { state: SearchState }) {
 ...
 <div key={i} className="bg-paper-sub rounded-xl border border-ink-rule p-6 animate-pulse">
 ...
 <button 
 onClick={() => window.location.reload()} 
 className="px-6 py-3 bg-amber-deep text-ink rounded-lg font-medium hover:bg-amber-deep/90"
 >
 Clear Filters
 </button>
 </div>
 ...
 {/* Pagination */}
 {pages > 1 && (
 <div className="flex items-center justify-between pt-6 border-t border-ink-rule">
 ...
 className="px-4 py-2 rounded-lg border border-ink-rule bg-paper-sub text-ink hover:bg-text-secondary/5 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
 >
 Previous
 </button>
