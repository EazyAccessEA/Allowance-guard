...
Skills live under `.claude/skills/` (support-triage, docs-coherence-audit, finance-snapshot, vendor-review, incident-postmortem, internal-coordination-brief). Autonomy level 2 across the board — briefs / audits / snapshots / reviews / post-mortems. The operator alone touches money, vendors, contracts, disclosures. `incident-postmortem` is the one append-allowed skill — it appends to `memory/product-engineering/incident-history.md` (a memory file, not `src/`) per operator approval. Ops Council (#36 Operations lead, #10 DevOps, #11 Investor voice) + #4 + #9 + #19 + #24 (VETO on data-processor changes / disclosure naming) + #1 + #14 + #35 convenes on every skill run. `claim-review` is **MANDATORY** before any external publication; `webhook-review` is **MANDATORY** for incident post-mortems touching webhooks; sub-processor changes route through `legal-page-draft` (Level 1, #24 VETO).
...
- 2026-04-17: Added Admin / Operations managed-agent system (pilot 6 — final of 6 pilots). 6 skills (support-triage, docs-coherence-audit, finance-snapshot, vendor-review, incident-postmortem, internal-coordination-brief) + 6 scoped memory files (support-categories, docs-map, vendor-register, finance-baseline, ops-calendar) + runbook. Ops Council leads (#36 Operations, #10 DevOps, #11 Investor voice). Operator alone touches money, vendors, contracts, disclosures. `incident-postmortem` appends to `memory/product-engineering/incident-history.md` per operator approval. The back-office: keeps the rest of the company functioning. **All 6 dept pilots now complete.**
